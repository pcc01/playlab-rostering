...
function mockPlaylab() {
  let id = 1;
  const calls = { createOrg:0, updateOrg:0, deactivateOrg:0, createUser:0, updateUser:0, suspendUser:0, createClass:0, updateClass:0 };
  ...
  const client = {
    ...
    createClass: async () => { calls.createClass++; return `playlab-class-${id++}`; },
    updateClass: async () => { calls.updateClass++; },
    healthCheck: async () => true,
...
describe('Normalizers — Clever', () => {
  ...
  test('normalizeCleverUser student: PII fields and FERPA', () => {
    ...
    assert.equal(entity.playbabRole, 'student');
  ...
  test('normalizeCleverSection: class + enrollments', () => {
    ...
    assert.equal(cls.entity.entityType, 'class');
...
describe('SyncWorker — initial provisioning', () => {
  test('creates orgs, users, classes in correct order with zero errors', async () => {
    ...
    assert.equal(calls.createClass, 1);
    assert.equal(r.orgsCreated, 2);
    assert.equal(r.usersCreated, 2);
    assert.equal(r.classesCreated, 1);
  });
...
describe('SyncWorker — error resilience', () => {
  test('records error and continues syncing remaining entities', async () => {
    ...
    const faultyClient = {
      ...
      updateUser: async () => {},
      suspendUser: async () => {},
      createClass: async () => `playlab-class-${idCounter++}`,
      updateClass: async () => {},
      healthCheck: async () => true,
    } as unknown as PlaybabClient;
