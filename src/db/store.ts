...
export class InMemoryStore implements DeduplicatorStore {
  readonly orgs = new Map<string, CanonicalOrganization>();           // canonicalId → org
  readonly users = new Map<string, CanonicalUser>();
  readonly classes = new Map<string, CanonicalClass>();
  ...
  async upsertEntity(entity: CanonicalEntity): Promise<void> {
    switch (entity.entityType) {
      ...
      case 'class': {
        const cls = entity as CanonicalClass;
  ...
  getAllClasses(): CanonicalClass[] { return [...this.classes.values()]; }
