 ...
 */
import { SourceConnector, CanonicalEntity, CanonicalWorkspace, EntityType } from '../types/canonical';
...
export class IngestPipeline {
  ...
  async run(opts: IngestOptions = {}): Promise<IngestResult> {
    ...
    const types = opts.entityTypes ?? ['organization', 'academicSession', 'workspace', 'user', 'enrollment'];
    ...
    try {
      ...
      if (types.includes('workspace')) await this.ingestClasses(result, opts);
