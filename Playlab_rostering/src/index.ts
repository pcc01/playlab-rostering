...
import { CanvasLtiConnector } from './connectors/canvas-lti';
...
const cfg = {
  ...
  canvasConsumerKey:   process.env.CANVAS_CONSUMER_KEY  ?? '',
  canvasSecret:        process.env.CANVAS_SECRET         ?? '',
  canvasDeveloperKey:  process.env.CANVAS_DEVELOPER_KEY  ?? '',
  canvasLtiVersion:    (process.env.CANVAS_LTI_VERSION   ?? '1.1') as '1.1'|'1.3',
  // Playlab
...
function buildConnectors(): SourceConnector[] {
  ...
  if (cfg.source === 'canvas' || cfg.source === 'all') {
    if (!cfg.canvasBaseUrl) { log.warn('CANVAS_BASE_URL not set — skipping Canvas'); }
    else connectors.push(new CanvasLtiConnector({
      clientId: cfg.canvasConsumerKey, clientSecret: cfg.canvasSecret,
      baseUrl: cfg.canvasBaseUrl, canvasBaseUrl: cfg.canvasBaseUrl,
      ltiConsumerKey: cfg.canvasConsumerKey, ltiSecret: cfg.canvasSecret,
      developerKey: cfg.canvasDeveloperKey || undefined,
      ltiVersion: cfg.canvasLtiVersion,
    }));
  }
  return connectors;
}
